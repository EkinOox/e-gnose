...
require_once("../model/livres_model.php");
?>
<!doctype html>
<html lang="fr">
...
<head>
    ...
    <meta name="description"
          content="livres, livres, audios | Toute une bibliothéque pour vous divertir, oé que vous soyez, en illimité !"/>
...
$sql = "SELECT * FROM livres WHERE id_livre = :id"; // :id = sécurité

// On prépare la requête
$requete = $db->prepare($sql);

// On injecte les paramétres en transformant les informations en entier
...
// On change le nom de l'onglet et des métadonnées en fonction du nom du livre
echo "<title>" . $media['livre_titre'] . " | e-Gnose" . "</title>";
echo "<meta property='og:title' content='" . $media['livre_titre'] . " | e-Gnose'.' />";
echo "<meta name='twitter:title' content='" . $media['livre_titre'] . " | e-Gnose'.' />";
?>

<section style="padding-bottom: 0;">
    <div>
        <?php
        if ($media['livre_value'] == 1 && $media['id_livre'] == $id) {
            $livre->getlivre(); ?>
        <?php } else {
            header("Location: ../index.php");
            exit;
        ...
        <div class="tabs">
            ...
            <div id="panel-details" role="tabpanel" tabindex="0" aria-labelledby="tab-1"
                ...
                <section>
                    <h3>Description :</h3>
                    <?php
                    $desc = $db->prepare('SELECT livres.livre_description FROM livres WHERE id_livre = ?');
                        ...
                        while ($descp = $desc->fetch(PDO::FETCH_ASSOC)) { ?>
                            ...
                            <div>
                                <div class="item__description">
                                    <p style="color:#fff;"><?= $descp['livre_description'] ?></p>
                                </div>
                            </div>
                            <?php
                        }
